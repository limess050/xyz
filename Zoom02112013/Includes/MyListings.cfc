...
<cffunction name="Get" access="remote" returntype="string" displayname="Creates the HTML to be displayed in the MyAccount template">
	...
	<cfelse>
		<cfquery name="getListingsOrders" datasource="#Request.dsn#" username="#Request.dbusername#" password="#Request.dbpassword#">
			Select LROV.ListingID, ROV.MyAccountInfo, ROV.OrderType, ROV.ListingFee, ROV.ExpandedListingFee, ROV.TotalListingFee,
			...
			Where LROV.ListingID in (<cfqueryparam value="#ValueList(getMyListings.ListingID)#" cfsqltype="CF_SQL_INTEGER" list="true">)
		</cfquery>
		<cfsavecontent variable="rString">
			 <table width="705" border="0" cellspacing="0" cellpadding="0" class="listingstable">
				...
				<cfoutput query="getMyListings">					
					<cfquery name="getListingOrders" dbtype="query">
						Select MyAccountInfo, OrderType, ListingFee, ExpandedListingFee, TotalListingFee,
						SepExpandedListingFee,
						RenewalListingFee, RenewalExpandedListingFee, RenewalTotalListingFee,
						ServiceOrderFee
						From getListingsOrders
	...
	<cfif checkListing.RecordCount>
		<cfif checkListing.InProgress>
			...
			<cfif Len(checkListing.ExpandedListingPDF)>
				<cfif FileExists("#Request.ListingUploadedDocsDir#\#checkListing.ExpandedListingPDF#")>
					<cffile action="delete" file="#Request.ListingUploadedDocsDir#\#checkListing.ExpandedListingPDF#">	
				</cfif>
			</cfif>
			<cfif Len(checkListing.UploadedDoc)>
				<cfif FileExists("#Request.ListingUploadedDocsDir#\#checkListing.UploadedDoc#")>
					<cffile action="delete" file="#Request.ListingUploadedDocsDir#\#checkListing.UploadedDoc#">	
				</cfif>
			</cfif>
			<cftransaction>
