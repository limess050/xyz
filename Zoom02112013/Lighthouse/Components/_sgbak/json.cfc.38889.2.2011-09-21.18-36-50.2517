...
Modifications made to work better with dojo by David Hammond (dave@modernsignal.com)
--->
...
<cfcomponent displayname="JSON" output="No">
	...
	<cffunction name="encode" hint="Converts data from CF to JSON format" access="remote" returntype="string" output="No">
		...
		--->
		<cfargument name="queryFormat" type="string" default="query" />
		<cfargument name="queryKeyCase" type="string" default="lower" />
		<cfargument name="queryIncludeMetadata" type="boolean" default="true" />
		...
		<cfset Var ColumnNames = StructNew()>
		...
		<!--- NUMBER --->
		<!--- Want numbers to be treated like strings
		...
		<cfelseif IsStruct(_data)>
			...
			<cfloop from="1" to="#ArrayLen(arKeys)#" index="i">
				...
				<cfif tempVal is not "unknown-obj">
					<!--- UCase keys to make ColdFusion 9 act the same as ColdFusion 8 --->
					<cfset jsonString = ListAppend(jsonString, '"' & UCase(arKeys[i]) & '":' & tempVal, ",") />
				</cfif>
			</cfloop>
		...
		<cfelseif IsQuery(_data)>
			...
			<!--- Create structure to hold case-sensitive column names --->
			<cfif StructKeyExists(Arguments,"columnList")>
				<cfloop list="#Arguments.columnList#" index="colname">
					<cfset ColumnNames[colname] = colname>
				</cfloop>
			</cfif>
			...
			<cfloop list="#columnList#" index="colname">
				<cfif Not StructKeyExists(ColumnNames,colname)>
					<cfset ColumnNames[colname] = colname>
				</cfif>
			</cfloop>
			...
			<cfif arguments.queryFormat EQ "query">
				...
				<cfloop list="#_data.columnlist#" delimiters="," index="column">
					...
					<cfset jsonString = jsonString & '"' & ColumnNames[column] & '":[' />
			...
			<cfelse>
				...
				<cfloop query="_data">
					...
					<cfloop list="#columnlist#" delimiters="," index="column">
						...
						<cfset jsonString = jsonString & ColumnNames[column] & ':' & tempVal />
			...
			<cfif queryIncludeMetadata>
				<!--- Add query meta data --->
				<!--- Wrap all query data into an object --->
				<cfif arguments.queryKeyCase EQ "lower">
					<cfreturn '{"recordcount":' & _data.recordcount & ',"columnlist":"' & columnlist & '"' & ',"data":' & jsonString & '}' />
				<cfelse>
					<cfreturn '{"RECORDCOUNT":' & _data.recordcount & ',"COLUMNLIST":"' & columnlist & '"' & ',"DATA":' & jsonString & '}' />
				</cfif>
			<cfelse>
